...
with tab3:
    ...
    if st.button("Predict"):
        try:
            ...
            if prediction == 1:
                st.error(f"High Risk: The model predicts a high likelihood of {disease} recurrence ).")
                st.write("Please consult a doctor immediately for further evaluation.")
            else:
                st.success(f"Low Risk: The model predicts a low likelihood of {disease} recurrence).")
